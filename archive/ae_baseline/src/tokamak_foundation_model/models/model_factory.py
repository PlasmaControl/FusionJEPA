...
def _vae_factory(ae_cls):
    """Return a callable that builds a VAE-wrapped instance of
    *ae_cls*.  Accepts the same kwargs as the underlying AE class."""
    def build(**kwargs):
        return VariationalWrapper(ae_cls(**kwargs))
    return build
...
MODEL_REGISTRY = {
    ...
    # Variational variants — drop-in replacements wrapping each AE
    # above.  See `VariationalWrapper` docstring.
    "fast_time_series_vae": _vae_factory(FilterscopeBaselineAutoEncoder),
    "slow_time_series_vae": _vae_factory(SlowTimeSeriesBaselineAutoEncoder),
    "profile_vae": _vae_factory(SpatialProfileBaselineAutoEncoder),
    "spectrogram_vae": _vae_factory(SpectrogramBaselineAutoEncoder),
    "spectrogram_tf_attn_vae": _vae_factory(SpectrogramTFOnlyAutoEncoder),
    "spectrogram_channel_ast_vae": _vae_factory(SpectrogramChannelASTAutoEncoder),
    "video_vae": _vae_factory(VideoBaselineAutoEncoder),
}
