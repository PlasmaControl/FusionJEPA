...
def main():
    ...
    parser.add_argument(
        "--n_tokens", type=int, default=32,
        help="Number of latent tokens (default: 32)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--weight_decay", type=float, default=0.3, help="AdamW weight decay"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--temporal_lambda", type=float, default=0.0,
        help="Weight for temporal metric-matching loss (0 disables)"
    )
    parser.add_argument(
        "--vae", action="store_true", default=False,
        help="Use variational autoencoder instead of plain AE"
    )
    parser.add_argument(
        "--vae_beta", type=float, default=1e-4,
        help="KL weight for VAE (only used when --vae is set)"
    )
    args = parser.parse_args()

    use_vae = args.vae
    vae_beta = args.vae_beta if use_vae else 0.0
    use_temporal = args.temporal_lambda > 0.0
    chunk_s = 0.1 if use_temporal else 0.05
    cache_suffix = "_pair" if use_temporal else ""
    ckpt_suffix = "_temporal" if use_temporal else ""
    if use_vae:
        ckpt_suffix = ckpt_suffix + "_vae"

    ### Paths ###
    signal_name = args.signal
    model_name = args.model or SIGNAL_MODEL_DEFAULTS[signal_name]
    if use_vae:
        model_name = model_name + "_vae"
    data_dir = Path(args.data_dir)
    statistics_path = Path(args.stats_path)
    checkpoint_path = (
            Path(args.checkpoint_dir)
            / f"{signal_name}_{model_name}{ckpt_suffix}"
            / "checkpoint.pth"
    )
    ...
    train_dataset = TokamakMultiFileDataset(
        train_paths,
        lengths_cache_path=f"lengths_train{cache_suffix}.pt",
        **shared_kwargs
    )
    validation_dataset = TokamakMultiFileDataset(
        val_paths,
        lengths_cache_path=f"lengths_validation{cache_suffix}.pt",
        **shared_kwargs
    )
    test_dataset = TokamakMultiFileDataset(
        test_paths,
        lengths_cache_path=f"lengths_test{cache_suffix}.pt",
        **shared_kwargs
    )
    ...
    logger.info(f"Sample data shape: {sample_data.shape}, "
                f"n_channels: {n_channels}, input_length: {input_length}"
                )
