"""Visualize ECE Channel-AST (no FSQ) reconstruction for a single shot.

Produces two figures:
  1. Original / Reconstructed / Error for a subset of channels
  2. Training + validation loss curves

Usage:
    module load pixi && pixi run python scripts/visualize_ece_channel_ast.py
"""
...
# == Config ================================================================
CHECKPOINT = Path(
    "runs/ece_channel_ast_nofsq_fw16"
    "/ece_spectrogram_channel_ast_fsq/checkpoint.pth"
)
...
SHOT = 201423
SAMPLE_IDX = 4
PLOT_CHANNELS = 8  # Show this many evenly-spaced channels (ECE has 40)
...
# -- Figure 1: Reconstruction (subset of channels) ------------------------
show_channels = np.linspace(0, n_channels - 1, min(PLOT_CHANNELS, n_channels), dtype=int)
...
fig.suptitle(
    f"ECE Channel-AST (shot {SHOT}, chunk {SAMPLE_IDX}, "
    f"{n_channels} ch, showing {n_rows}) — {subtitle}",
    fontsize=11,
)
fig.tight_layout()
...
ax_loss.legend()
ax_loss.set_title("ECE Channel-AST (no FSQ, fw=16) — Loss Curves")
fig2.tight_layout()
